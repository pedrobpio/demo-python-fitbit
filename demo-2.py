...
if __name__ == '__main__':
	...
	#open list of users register
	file = open('user.txt', 'r')
	user = None
	numberLine = 1
	NumberLineMax =  sum(1 for line in open('user.txt'))#get number of users register
	#get the user activity and write its steps count for each user in the list of users
	while numberLine <= NumberLineMax:
		user = getUser(numberLine)
		activities = getActivitiesBasic(user)
		print ("Hi, "+ user + " you've walked " + str(activities.steps) +" steps today")
		numberLine += 1
		pass
